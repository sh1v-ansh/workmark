...
import type { Company, Project, Application, GithubEvidencedSkill } from '@/lib/types'
...
function NewProjectForm({ companyId, companyName, onCreated, onCancel }: {
  companyId: string; companyName: string | null; onCreated: (p: Project) => void; onCancel: () => void
}) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const { data, error } = await supabase
        .from('projects')
        .insert({
          poster_id: companyId, poster_type: 'company',
          poster_display_name: companyName,
...
export default function CompanyDashboardClient({ company, initialProjects, initialApplications, githubSkillsByStudent }: Props) {
  const { toast } = useToast()
  ...
  async function handleAccept(appId: string) {
    ...
    try {
      ...
      const { error: expErr } = await supabase.from('verified_work_records').insert({
        ...
        poster_id: company.id, poster_type: 'company',
        project_id: app.project_id, project_title: project?.title ?? null,
        poster_display_name: company.company_name,
