  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
        ...
        <button type="button" onClick={add} style={{ padding: '0 14px', background: C.surfaceAlt, border: `1px solid ${C.border}`, color: C.textMuted, fontFamily: F.mono, fontSize: 11, cursor: 'pointer' }}>Add</button>
      </div>
...
function NewProjectForm({ facultyId, institution, onCreated, onCancel }: {
  ...
  const [location, setLocation] = useState(institution ?? '')
  ...
  const [scope, setScope] = useState<'institution' | 'open'>('institution')
  const [scopedTo, setScopedTo] = useState(institution ?? '')
    ...
    try {
      const { data, error } = await supabase
        ...
        .insert({
          ...
          scoped_to_institution: scope === 'institution' ? scopedTo : null,
        })
        .select().single()
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={fieldGap}>
        ...
        <input id="proj-title" required value={title} onChange={(e) => setTitle(e.target.value)} className="dk-input" placeholder="NLP Research Assistant" />
      </div>

      <div style={fieldGap}>
        <Label htmlFor="proj-desc">Description</Label>
        <textarea id="proj-desc" value={description} onChange={(e) => setDescription(e.target.value)} rows={4} className="dk-textarea" placeholder="Describe the research project, goals, and what students will learn…" />
      </div>

      {/* Student scope */}
      <div style={{ background: C.surfaceAlt, border: `1px solid ${C.border}`, padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
        <p style={{ fontFamily: F.mono, fontSize: 10, color: C.textFaint, letterSpacing: '0.1em', textTransform: 'uppercase' }}>Student eligibility</p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {([
            { v: 'institution', label: 'My institution only', sub: 'Only students at your university can apply' },
            { v: 'open', label: 'All students', sub: 'Any student on Workmark can apply' },
          ] as { v: 'institution' | 'open'; label: string; sub: string }[]).map(({ v, label, sub }) => (
            <label key={v} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, cursor: 'pointer' }}>
              <input type="radio" name="scope" value={v} checked={scope === v} onChange={() => setScope(v)} style={{ accentColor: C.accent, marginTop: 2 }} />
              <div>
                <p style={{ fontSize: 13, color: C.textSub }}>{label}</p>
                <p style={{ fontSize: 11, color: C.textFaint, fontFamily: F.mono }}>{sub}</p>
              </div>
            </label>
          ))}
        </div>
        {scope === 'institution' && (
          <div style={fieldGap}>
            <Label htmlFor="proj-scoped-to">Institution</Label>
            <Combobox id="proj-scoped-to" value={scopedTo} onChange={setScopedTo} options={UNIVERSITIES} placeholder="Search universities…" />
          </div>
        )}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        ...
        <div style={fieldGap}>
          ...
          <input id="proj-location" value={location} onChange={(e) => setLocation(e.target.value)} className="dk-input" placeholder="Lab / building name" />
        </div>
        <div style={fieldGap}>
          <Label htmlFor="proj-duration">Duration</Label>
          <input id="proj-duration" value={duration} onChange={(e) => setDuration(e.target.value)} className="dk-input" placeholder="1 semester, 10 weeks…" />
        </div>
        <div style={fieldGap}>
          <Label htmlFor="proj-hours">Hours / week</Label>
          <input id="proj-hours" type="number" min={1} max={40} value={hoursPerWeek} onChange={(e) => setHoursPerWeek(e.target.value)} className="dk-input" placeholder="10" />
        </div>
        <div style={fieldGap}>
      ...
      <div style={{ background: C.surfaceAlt, border: `1px solid ${C.border}`, padding: 16 }}>
        <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }}>
          <input type="checkbox" checked={isPaid} onChange={(e) => setIsPaid(e.target.checked)} className="dk-checkbox" />
          <span style={{ fontSize: 13, color: C.textMuted }}>This position includes a stipend or pay</span>
        </label>
        {isPaid && (
          <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 6 }}>
            <Label htmlFor="proj-comp">Compensation details</Label>
            <input id="proj-comp" value={compensation} onChange={(e) => setCompensation(e.target.value)} className="dk-input" placeholder="$3000 stipend, course credit…" />
          </div>
        )}
      </div>

      <TagInput label="Required skills" inputId="proj-req-skills" value={reqSkills} onChange={setReqSkills} placeholder="Python, PyTorch…" />
      <TagInput label="Preferred skills (optional)" inputId="proj-pref-skills" value={prefSkills} onChange={setPrefSkills} placeholder="CUDA, HuggingFace…" />

      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        <Label htmlFor="proj-majors-cb">Preferred majors (optional)</Label>
        <Combobox id="proj-majors-cb" value='' onChange={(v) => { if (!prefMajors.includes(v)) setPrefMajors([...prefMajors, v]) }} options={MAJORS.filter((m) => !prefMajors.includes(m))} placeholder="Search majors to add…" />
        {prefMajors.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 4 }}>
            {prefMajors.map((m) => (
              <span key={m} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '2px 8px', background: C.surfaceAlt, border: `1px solid ${C.border}`, fontSize: 11, color: C.textSub, fontFamily: F.mono }}>
                {m}
                <button type="button" onClick={() => setPrefMajors(prefMajors.filter((x) => x !== m))} aria-label={`Remove ${m}`} style={{ background: 'none', border: 'none', color: C.textFaint, cursor: 'pointer', padding: 0, lineHeight: 1 }}>
                  <span aria-hidden="true">×</span>
                </button>
              </span>
            ))}
...
export default function FacultyDashboardClient({ faculty, initialProjects, initialApplications }: Props) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg }}>
      <Navbar role="faculty" userName={faculty.full_name ?? undefined} />
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16 }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: C.textMuted, fontFamily: F.mono }}>
              {[faculty.title, faculty.department, faculty.institution].filter(Boolean).join(' · ')}
            </p>
          </div>
        ...
        <section>
          ...
          {projects.length === 0 ? (
            <div style={{ background: C.surface, border: `1px solid ${C.border}`, padding: 40, textAlign: 'center' }}>
              ...
              <p style={{ fontSize: 12, color: C.textFaint, fontFamily: F.mono }}>Click &ldquo;Post project&rdquo; to find students for your research.</p>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', marginBottom: 4 }}>
                            ...
                            <span style={{ fontSize: 9, fontFamily: F.mono, padding: '1px 6px', textTransform: 'uppercase', letterSpacing: '0.08em', color: project.is_open ? C.accent : C.textFaint, border: `1px solid ${project.is_open ? C.accentBorder : C.border}`, background: project.is_open ? C.accentHover : C.surfaceAlt }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                          ...
                          <button onClick={() => setExpandedProject(expanded ? null : project.id)}
                            ...
                            {pendingCount > 0 && (
                              <span style={{ width: 16, height: 16, background: C.accent, color: C.bg, borderRadius: '50%', fontSize: 9, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700 }} aria-label={`${pendingCount} pending`}>{pendingCount}</span>
                            )}
